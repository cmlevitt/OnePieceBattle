...
bool Shape::isOverlapping(const vec2 &point) const {
    ...
    if (point.x > getLeft() && point.x < getRight())// point (mouse position) is between left and right sides of rect
        if (point.y < getTop() && point.y > getBottom())// point is between top and bottom sides of rect
            return true;
    return false;
}
